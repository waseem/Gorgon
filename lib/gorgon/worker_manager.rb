...
module Gorgon
  class WorkerManager
    ...
    def initialize config
      @logger = GLogger.new(config[:log_file])
      @logger.log "Worker Manager #{Gorgon::VERSION} initializing"
    ...
    def fork_workers n_workers
      logger.log "Running before_creating_workers callback"
      @callback_handler.before_creating_workers

      logger.log "Forking #{n_workers} worker(s)"
      EventMachine.run do
        n_workers.times do
          fork_a_worker
        end
    ...
    def fork_a_worker
      ...
      loggger.log "Forking Worker #{worker_id}"
      ...
      watcher = proc do
        ...
        logger.log "Worker #{pid} finished"
        status
      end
      ...
      worker_complete = proc do |status|
        if status.exitstatus != 0
          ...
          logger.log_error "Worker #{pid} crashed with exit status #{exitstatus}!"
          ...
          begin
            ...
            logger.log_error "Exception raised when trying to report crash to originator:"
    ...
    def on_current_job_complete
      logger.log "Job '#{@job_definition.inspect}' completed"
    ...
    def subscribe_to_originator_queue
      ...
      handle_message = proc do |payload|
        if payload[:action] == "cancel_job"
          logger.log "Cancel job received!!!!!!"

          logger.log "Sending 'INT' signal to #{@worker_pids}"
